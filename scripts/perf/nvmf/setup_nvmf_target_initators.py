...
#        self.log_print("cebruns - NOT Zipping SPDK source directory")
    ...
    def read_json_stats(self, file):
        ...
        return [disks, read_iops, read_bw, read_avg_lat, read_min_lat, read_max_lat, read_p99_lat,
    ...
    def parse_results(self, results_dir, initiator_count=None, run_num=None):
        ...
        with open(os.path.join(results_dir, csv_file), "w") as fh:
            header_line = ",".join(["TestName",
                                    "disks", "read_iops", "read_bw_kb", "read_avg_lat_us",
                                    "read_min_lat_us", "read_max_lat_us", "read_p99_lat_us",
                                    "write_iops", "write_bw_kb", "write_avg_lat_us",
        ...
        for fio_config in fio_files:
            ...
            for i in inits_names:
                ...
                self.log_print("[disks, read_iops, read_bw_kb, read_avg_lat, read_min_lat, read_max_lat, "
                                "read_p99_lat, write_iops, write_bw_kb, write_avg_lat, write_min_lat,"
                                "write_max_lat, write_p99_lat]")
                ...
                # cebruns - TODO do something here if we want per-initiator stats
                self.log_print(z)
...
class Initiator(Server):
    ...
    def discover_subsystems(self, address_list, subsys_no):
        self.log_print("Address_list: %s" % address_list)
        self.log_print("nvmecli_bin: %s" % self.nvmecli_bin)
        num_nvmes = range(0, subsys_no)
        nvme_discover_output = ""
        for ip, subsys_no in itertools.product(address_list, num_nvmes):
            self.log_print("Trying to get hostname")
            test_command = "hostname"
            stdout, stderr = self.remote_call(test_command)
            self.log_print("hostname stdout: %s" % stdout)
            ...
            self.log_print("discover stdout: %s" % stdout)
            self.log_print("discover stderr: %s" % stderr)
            if stdout:
...
"""
        if "spdk" in self.mode:
            ...
            self.log_print("Subsystems: %s" % subsystems)
            if not subsystems:
                self.log_print("No subsystems found - exiting!")
                # Maybe better cleanup TODO: cebruns
                exit(-1)
        ...
        fio_config_filename = "%s_%s_bs_%s_qd_%s_read_pct" % (rw, block_size, io_depth, rwmixread)
        ...
        self.log_print("Created FIO Config in %s" % self.spdk_dir)
#        self.log_print(fio_config)

        return os.path.join(self.spdk_dir, "nvmf_perf", fio_config_filename)

    def run_fio(self, fio_exe, fio_config_file, run_num=None):
        job_name, _ = os.path.splitext(fio_config_file)
        self.log_print("Starting FIO run for job: %s and run_num: %s" % (job_name, run_num))
        if run_num:
            for i in range(1, run_num + 1):
                output_filename = job_name + "_run_" + str(i) + "_" + self.name + ".json"
                cmd = "sudo %s %s --output-format=json --output=%s" % (fio_exe, fio_config_file, output_filename)
                self.log_print("Command to run remotely: %s" % cmd)
                output, error = self.remote_call(cmd)
                if output:
                    self.log_print("Output from fio remote call: %s" % output)
                if error:
                    self.log_print("Err from fio remote call: %s" % error)
        else:
            output_filename = job_name + "_" + self.name + ".json"
            cmd = "sudo %s %s --output-format=json --output=%s" % (fio_exe, fio_config_file, output_filename)
            output, error = self.remote_call(cmd)
            self.log_print("Output from non run_num fio remote call: %s" % output)
            self.log_print("Err from non run_num fio remote call: %s" % error)
...
class SPDKTarget(Target):
    def __init__(self, name, username, password, mode, nic_ips, num_cores, num_shared_buffers=4096,
        ...
        self.max_qpairs_per_ctrlr = max_qpairs_per_ctrlr
        self.num_null_devices = num_null_devices
        self.log_print("Num_Null_Devices to be created: %s" % num_null_devices)
        self.log_print("Num_cores to be used: %s" % self.num_cores )
        self.log_print("Max_Qpairs to be created per controller: %s" % max_qpairs_per_ctrlr)
    ...
    def spdk_tgt_configure(self):
        ...
        rpc.nvmf.nvmf_create_transport(self.client, trtype=self.transport, num_shared_buffers=self.num_shared_buffers, max_qpairs_per_ctrlr=self.max_qpairs_per_ctrlr)
        ...
        if self.null_block:
            ...
            subsystems_section = self.spdk_tgt_add_subsystem_conf(self.nic_ips, req_num_disks=self.num_null_devices)
        else:
    ...
    def spdk_tgt_add_nullblock(self):
        ...
        # cebruns - create requested number of null bdevs
        for i in range(0, self.num_null_devices):
            dev_name = "Nvme{0}n1".format(i)
            rpc.bdev.construct_null_bdev(self.client, 102400, 4096, dev_name)
    ...
    def spdk_tgt_add_subsystem_conf(self, ips=None, req_num_disks=None):
        ...
        for ip, chunk in zip(ips, disk_chunks):
            for c in chunk:
                nqn = "nqn.2018-09.io.spdk:cnode%s" % (c - 1)
                serial = "SPDK00%s" % (c - 1)
    ...
    # cebruns - we define cores in json but need mask for SPDK/DPDK
    def convert_cores_to_mask(self,cores):
        mask = 0
        # Have to use at least one core to run SPDK
        if cores == 0:
            cores = 1
    ...
    def tgt_start(self):
        ...
        self.log_print("Starting SPDK NVMeOF Target process with subsysNo: %s" % self.subsys_no)
        nvmf_app_path = os.path.join(self.spdk_dir, "app/nvmf_tgt/nvmf_tgt")
        coremask = self.convert_cores_to_mask(int(self.num_cores))
        command = " ".join([nvmf_app_path, "-m", coremask])
        print ("Target command is: {}".format(command))
...
class KernelInitiator(Initiator):
    ...
    def kernel_init_connect(self, address_list, subsys_no):
        self.log_print("Kernel_init_connect")
        ...
        self.log_print("Command: %s" % self.nvmecli_bin)
        for subsystem in subsystems:
            self.log_print("Trying to connect %s %s %s" % subsystem)
            out, err = self.remote_call("sudo %s connect -t %s -s %s -n %s -a %s -i 8" % (self.nvmecli_bin, self.transport, *subsystem))
            if out:
                self.log_print("Connect stdout: %s" % out)
            if err:
                self.log_print("Connect stderr: %s" % err)
            time.sleep(2)
    ...
    # cebruns - updated to look for only test against BDev controllers
    def gen_fio_filename_conf(self):
        out, err = self.remote_call("lsblk -o NAME,SERIAL,TYPE,MODEL,SIZE -nlp")
        nvme_list = [x for x in out.split("\n") if "SPDK bdev Controller" in x]
        self.log_print("NVME_List in gen_fio_filename_conf: %s" % nvme_list)

        filename_section = ""
        if len(nvme_list) is 0:
            print ("No SPDK BDevs Found - ABORTING")
            exit(-1)
...
class SPDKInitiator(Initiator):
    ...
    def install_spdk(self, local_spdk_zip):
        ...
        print ("Self SPDK dir is: {0}".format(self.spdk_dir))
        self.log_print("Sources unpacked")
        self.log_print("Building SPDK")
        out, err = self.remote_call("cd %s; git submodule update --init; ./configure --with-rdma --with-fio=/opt/fio; make clean; make -j$(($(nproc)*2))" % self.spdk_dir)
        print ("Building out: {}".format(out))
        print ("Building err: {}".format(err))
...
if __name__ == "__main__":
    ...
    for k, v in data.items():
        if "target" in k:
            ...
            if "fio_exe" in data[k].keys():
                fio_exe = data[k]["fio_exe"]
            else:
                print("Need to set fio_exe in config.json")
                exit(-1)
        else:
            continue
...
#            print("obj.%s = %r" % (attr, getattr(i, attr)))
    ...
    threads = []
    configs = []
    # cebruns - just take the first fio workload from the list
    for block_size, io_depth, rw in fio_workloads:
        ...
        # TODO: we will only gen an FIO file for the last config in a list of setups
        # Would it be better to signal to somebody that we're ready to run and sync here with
        # all other targets? RabbitMQ maybe?
